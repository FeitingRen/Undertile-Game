...
void DrawTextScaled(const char *text, int x, int y, Color color, float sizeMult = 1.0f)
{
    ...
    DrawTextEx(myCustomFont, text, pos, 35.0f * sizeMult, 2.0f, color);
}
...
void DrawSpeechBubble(const char *text, bool instant)
{
    ...
    if (instant)
    {
        DrawTextEx(myCustomFont, text, {bx + (5 * SCALE), by + (5 * SCALE)}, 35.0f, 2.0f, BLACK);
    }
    else
    {
        // Use Typewriter for non-instant
        globalTypewriter.Draw(myCustomFont, (int)(bx + 5 * SCALE), (int)(by + 5 * SCALE), 35.0f, 2.0f, BLACK);
    }
...
void InitBattle()
{
    ...
    globalTypewriter.Start("我討厭死咖啡了。", 30);
...
void UpdateBattle()
{
    ...
    case B_Q1_DIALOGUE:
        if (IsInteractPressed() && globalTypewriter.IsFinished())
        {
            ...
            // Script
            if (dialogueIndex == 1)
                globalTypewriter.Start("它的存在比人類還沒有意義。", 30);
            else if (dialogueIndex == 2)
                globalTypewriter.Start("用咖啡來讓本就超負荷的身體繼續工作？", 30);
            else if (dialogueIndex == 3)
                globalTypewriter.Start("為什麼人類這麼擅長折磨所有事物。", 30);
            else if (dialogueIndex == 4)
                globalTypewriter.Start("我曾經被人逼迫沒有意義地從1數到50億。", 30);
            else if (dialogueIndex == 5)
                globalTypewriter.Start("在那之後，我患上了精神分裂。", 30);
            else if (dialogueIndex == 6)
                globalTypewriter.Start("我經受過的折磨，現在該到你來感受了！", 30);
    ...
    case B_Q1_RESULT:
        ...
        if (battleTimer > 0.8f)
        {
            battlePhase = B_Q2_DIALOGUE;
            if (isCorrect)
                globalTypewriter.Start("謝謝！", 30);
            else
            {
                globalTypewriter.Start("AI真的又蠢又沒用。", 30);
    ...
    case B_Q2_RESULT:
        ...
        if (battleTimer > 0.8f)
        {
            battlePhase = B_Q3_DIALOGUE;
            if (isCorrect)
                globalTypewriter.Start("為什麼你說話跟我媽一樣？", 30);
            else
            {
                globalTypewriter.Start("你難道不能根據我的網絡ip去查一下我的\n天氣嗎？", 30);
                isCorrect = true;
            }
        }
        break;
    ...
    case B_Q3_RESULT:
        ...
        if (battleTimer > 0.8f)
        {
            battlePhase = B_Q4_DIALOGUE;
            if (isCorrect)
                globalTypewriter.Start("合同裡房子的地址和價格你為什麼沒寫？", 30);
            else
            {
                globalTypewriter.Start("每個月付你20塊錢，結果你\n連這都做不\n到？", 30);
                globalTypewriter.Start("你難道不能根據我的網絡ip去查一下我的\n天氣嗎？", 30);
                isCorrect = true;
            }
        }
    ...
    case B_Q4_RESULT:
        ...
        if (battleTimer > 0.8f)
        {
            battlePhase = B_Q5_DIALOGUE;
            if (isCorrect)
                globalTypewriter.Start("但他有時候對我真的挺好的。", 30);
            else
            {
                globalTypewriter.Start("你到底有沒有看我發的東西？", 30);
                isCorrect = true;
            }
        }
        break;
    ...
    case B_Q5_RESULT:
        ...
        if (battleTimer > 0.8f)
        {
            battlePhase = B_Q6_DIALOGUE;
            if (isCorrect)
                globalTypewriter.Start("那什麼股票明天會漲？", 30);
            else
            {
                globalTypewriter.Start("它明天的最低點和最高點會在哪一秒？", 30);
                isCorrect = true;
            }
        }
        break;
    ...
    case B_Q6_RESULT:
        ...
        {
            battlePhase = B_Q7_DIALOGUE;
            globalTypewriter.Start("你怎麼不說話？", 30);
        }
        break;
    ...
    case B_Q7_RESULT:
        ...
        if (battleTimer > 1.0f)
        {
            battlePhase = B_VICTORY;
            dialogueIndex = 0;
            globalTypewriter.Start("這新聞是锟届瀿锟斤拷��������", 30);
        }
        break;
    ...
    case B_VICTORY:
        if (IsInteractPressed() && globalTypewriter.IsFinished())
        {
            dialogueIndex++;
            if (dialogueIndex == 1)
                globalTypewriter.Start("這锟届瀿锟斤拷��������", 30);
            else if (dialogueIndex == 2)
                globalTypewriter.Start("锟届瀿锟斤拷����烫烫烫", 30);
            else if (dialogueIndex == 3)
                globalTypewriter.Start("锟届瀿锟斤拷����烫����烫烫烫", 30);
            else if (dialogueIndex == 4)
                globalTypewriter.Start("天哪！你還好嗎？", 30);
            else if (dialogueIndex == 5)
            {
                StopMusicStream(battleBGMusic);
                globalTypewriter.Start("對不起我喝完咖啡以後太上頭了", 30);
            }
            else if (dialogueIndex > 5)
            {
